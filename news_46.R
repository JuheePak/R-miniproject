...
final_tit<-c()
final_date<-c()
mydate <- c(rep(20200401:20200430, 1), rep(20200501:20200531, 1), rep(20200601:20200630, 1))
mydate <- mydate[-8] #4월8일데이터 제외
mydate
...
write.csv(final_data,"nate_news_46.csv",row.names = F)
save(final_data,file="nate_news_46.RData")
...
save(comments,file="pre_pro_news_46.RData")
length(comments)
...
stw<-c("으로","하는","에서","입니다","합니다","있습니다","하고","에게","있는","라고","에도","이고","하게","또한","하기",
       ...
       "무단","구독신청","hankyung","hankyung.com")

stw2<-c("네이버","일보","닷컴","금지","바로가기","모바일","news","보세요","YTN")
...
saveWorkbook(wb, "newsdata_46.xlsx")
